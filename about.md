---
layout: about
image: /assets/img/me/me.jpeg
description: >
  안녕하세요. 유종원입니다.
hide_description: true
---

# About
<!--author-->
***
<center>
<span style="font-size:170%;font-weight:bold"> 유종원
</span>
</center>
<center>Position : Back-End</center>

<!-- <center>Age : 28</center>
<center>Address : Byeolnae-dong, Namyangju-si, Gyeonggi-do</center> -->


<br>
<br>
<br>
<br>
<br>
<br>
## Skills and Certification

<center>
<h3>기술스택</h3><br>
  <img src="https://img.shields.io/badge/Java-007396?style=flat-square&logo=Java&logoColor=white"/>
  <img src="https://img.shields.io/badge/Spring-6DB33F?style=flat-square&logo=Spring&logoColor=white"/>
  <img src="https://img.shields.io/badge/SpringBoot-6DB33F?style=flat-square&logo=Spring Boot&logoColor=white"/>
  <img src="https://img.shields.io/badge/Javascript-F7DF1E?style=flat-square&logo=Javascript&logoColor=white"/>
  <img src="https://img.shields.io/badge/jQuery-0769AD?style=flat-square&logo=jQuery&logoColor=white"/><br>
  <img src="https://img.shields.io/badge/Thymeleaf-005F0F?style=flat-square&logo=Thymeleaf&logoColor=white"/>
  <img src="https://img.shields.io/badge/Oracle-F80000?style=flat-square&logo=Oracle&logoColor=white"/>
  <img src="https://img.shields.io/badge/MariaDB-003545?style=flat-square&logo=MariaDB&logoColor=white"/>
    <img src="https://img.shields.io/badge/Apache Tomcat-F8DC75?style=flat-square&logo=Apache Tomcat&logoColor=white"/>
    </center>


### 자기개발

링크 : https://velog.io/@yjw8459
깃허브 : 


<div class="list-post">

  
  
    
  <h2 class="list-lead"></h2> 
  <ul class="related-posts">

    <li class="h6">
      <div>
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <a href="/2022-02-02-hits/" class="flip-title"><span>2022.02 학점은행제 학사 취득</span></a>
      </div>
    </li>

    <li class="h6">
      <div>
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <a href="/2022-02-02-hits/" class="flip-title"><span>2020.09 피이지시스템 입사(현재)</span></a>
      </div>
    </li>

    <li class="h6">
      <div>
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <a href="/project/2022-01-31-markdown/" class="flip-title"><span>2020.08 자바 백엔드 교육과정 수료</span></a>
      </div>
    </li>  
    <li class="h6">
      <div>
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <a href="/project/2022-01-31-markdown/" class="flip-title"><span>2020.03 자바 백엔드 교육과정(에이콘 아카데미)</span></a>
      </div>
    </li>  

    <li class="h6">
      <div>
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <a href="/project/2022-01-31-markdown/" class="flip-title"><span>2020.02 서일대학교 졸업</span></a>
      </div>
    </li>  

    <li class="h6">
      <div>
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <a href="/project/2022-01-31-markdown/" class="flip-title"><span>2017.02 서일대학교 컴퓨터응용전자과 입학</span></a>
      </div>
    </li>  
  </ul>


<h2 class="list-lead">2021</h2>
<ul class="related-posts">
    
<li class="h6">
  <div>
    <time style="display: inline-block; width: 2.2rem" class="faded fine" datetime="2021-02-13T00:00:00+09:00">13 Feb</time>
    <a href="/subcat/test2/2021-02-13-subcat-test1-post2/" class="flip-title"><span>예시 게시물2 </span></a>
  </div>
</li>
<li class="h6">
  <div>
    <time style="display: inline-block; width: 2.2rem" class="faded fine" datetime="2021-02-13T00:00:00+09:00">13 Feb</time>
    <a href="/subcat/test2/2021-02-13-subcat-test1-post1/" class="flip-title"><span>예시 게시물2 </span></a>
  </div>
</li>

<li class="h6">
  <div>
    <time style="display: inline-block; width: 2.2rem" class="faded fine" datetime="2021-02-13T00:00:00+09:00">13 Feb</time>
    <a href="/subcat/test1/2021-02-13-subcat-test1-post2/" class="flip-title"><span>예시 게시물2 </span></a>
  </div>
</li>
  
 
    </ul>
    <h2 class="list-lead">2020</h2>
    <ul class="related-posts">
<li class="h6">
  <div>
    <time style="display: inline-block; width: 2.2rem" class="faded fine" datetime="2020-07-03T00:00:00+09:00">03 Jul</time>
    <a href="/example2/2020-07-03-example2-post1/" class="flip-title"><span>Introducing Hydejack 9 </span></a>
  </div>
</li>
  
    </ul>
    <h2 class="list-lead">2018</h2>
    <ul class="related-posts">
    
<li class="h6">
  <div>
    <time style="display: inline-block; width: 2.2rem" class="faded fine" datetime="2018-06-01T00:00:00+09:00">01 Jun</time>
    <a href="/project/2018-06-01-example-post3/" class="flip-title"><span>Example Content III </span></a>
  </div>
</li>
  

  
  
    </ul>
    <h2 class="list-lead">2017</h2>
    <ul class="related-posts">
    
  
  


<li class="h6">
  <div>
    <time style="display: inline-block; width: 2.2rem" class="faded fine" datetime="2017-11-23T00:00:00+09:00">23 Nov</time>
    <a href="/project/2017-11-23-example1-post2/" class="flip-title"><span>Example Content II </span></a>
  </div>
</li>
  

  
  
    </ul>
    <h2 class="list-lead">2012</h2>
    <ul class="related-posts">
    
  
  


<li class="h6">
  <div>
    <time style="display: inline-block; width: 2.2rem" class="faded fine" datetime="2012-02-07T00:00:00+09:00">07 Feb</time>
    <a href="/project/2012-02-07-example1/" class="flip-title"><span>Example1-post1 </span></a>
  </div>
</li>
  </ul>

</div>

## 피이지시스템

## 주요사업
<br>
## _연말정산 패키지 사업_
- 관리자 전용 CS 프로그램 
- 사용자 웹 프로그램 
  - API 개발
  - UI 개발
- 시스템 유지보수
  - 고객사 별 서버 구축 및 배포
  - 서버 관련 문의 대응
- 80개 이상 고객사 서비스

## _SI 개발 및 유지보수_
- Spring Framework
- JSP


## 주요 직책 및 역할

- API 서버 개발 
- 운영 환경 구축 및 배포 
- UI 기능 개발 




### KDIS (국가정책대학원) 
#### 서버
  - 전자정부 표준프레임워크를 기반으로 한 서버 개발 
  - MyBatis로 SQL, 저장 프로시저를 사용하여 데이터 엑세스
  - 인터셉터를 이용한 XSS 체크 
  - Junit4, MockMvc 기능 테스트 
  - SSO 포털 연동 로그인
  - 도메인 SSL 적용 및 리다이렉트 설정 

#### 클라이언트
  - jquery를 사용하여 UI 클라이언트 개발
  - 중복 코드를 줄이기 위한 템플릿/콜백 패턴 적용 

#### api
  - 이메일 발송
  - 문자메시지(크로샷), 알림톡(팝빌)
  - 레포트 뷰어



## 그 외  프로젝트 

### 금강대학교 MIS
- API 추가 개발 및 유지보수
- Spring Data JPA, My Batis를 사용한 데이터 엑세스 

### PEG 연말정산
- 일괄 문자 전송 서비스 개발 
- 연말정산 고객센터 개발
- 고객사 OS 별 구축 및 배포

### Toyo Korea 입사 지원관리



## 서버 개발자로써💻
### 자격사항
<br>
<div class="list-post">
  <ul class="related-posts">

    <li class="h6">
      <div>
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <a href="/2022-02-02-hits/" class="flip-title"><span>2022.03 SQLD 준비중</span></a>
      </div>
    </li>
    <li class="h6">
      <div>
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <a href="/2022-02-02-hits/" class="flip-title"><span>2022.01 리눅스 마스터 2급 필기 합격</span></a>
      </div>
    </li>
    <li class="h6">
      <div>
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <a href="/2022-02-02-hits/" class="flip-title"><span>2021.10 네트워크 관리사 2급 취득</span></a>
      </div>
    </li>
    <li class="h6">
      <div>
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <a href="/2022-02-02-hits/" class="flip-title"><span>2021.08 정보처리 산업기사 취득</span></a>
      </div>
    </li>

  </ul>

</div>



## 다양한 경험
- **_클라이언트/서버 개발_**
  - 
현 직장에서 백엔드 개발 업무를 맡고 있지만 프론트엔드 업무도 맡고 있어서 보다 다양한 업무를 접하였습니다.<br>
사내에 웹 개발자의 인원이 많지 않아서 프론트엔드, 백엔드의 직무가 구분되어 있지 않았습니다. <br>
그렇기에 자연스럽게 웹 개발에 있어서 넓은 범위의 업무를 담당하게 되었습니다.

넓은 범위를 담당하는 만큼 거부감도 있었습니다.<br>
한 분야의 개발이 아닌 웹 개발의 전체적인 부분을 담당하는 것이기에 전문성을 갖고자
알아야 할 것들도 많았습니다.<br>
전문성을 갖고자 하는 범위가 넓은 만큼 개인 시간도 개발에 몰두하며 기술적인 어려움을 겪기도 했지만, 
덕분에 넓은 분야의 기술에 관심을 갖고 즐길 수 있게 되었습니다.

## 다양한 경험
<br>
- **_OS별 서버 구축_**
  - 연말정산 60여 곳 배포 
  - 신규업체 서버 구축

연말정산 고객사 60여곳의 배포 및 신규업체 구축을 하며, 다양한 네트워크나 OS에 대한 지식을 접할 수 있었습니다.
<br><br>
- **_리팩터링_**
  - 느슨한 결합, 강한 응집도를 위한 관심사 분리
  - 재사용 가능한 코드를 작성하기위해 고려

여러 프로젝트를 접하면서 중복되는 코드나 패턴들이 눈에 들어왔습니다.<br>
로직 여러 곳에서 중복되는 수정자 코드나, 유효성 검사 로직, 예외처리 로직이었습니다.<br>
이러한 제 개발 방식에 회의감을 느끼고, 내가 스프링에 대해 얼마나 아는가에 대해 다시 고민해볼 수 있었습니다.<br>
스프링을 다시 공부하며 응집도는 높이고 결합은 느슨하게 하였고, 객체들의 관심사를 분리하여 
재사용 가능한 코드인가를 고려하게 되었습니다.


## 사이드 프로젝트🧾

- #### _업무에 대한 기본 지식 학습_
- #### _기존 코드 분석을 통한 업무 체득화_
- #### _지속적인 통합 경험_ 
- #### _글로벌에 걸맞는 개발자를 목표_
<br>

오스템에 입사하게 된다면 가장 먼저 오스템에서 진행하는 직무 교육에 집중하여 업무에 대한 기본 지식을 키우겠습니다.<br>
이 후 기존에 다뤄보지 못했던 웹 개발에 대한 개발 능력을 키우기 위해 관련 라이브러리 학습과 기존 코드에 대한 분석을 통해 간단한 과제나 프로젝트를 진행하여 업무에 적응하도록 하겠습니다.<br>
또, 끊임없는 자기계발과 경험을 통해 직무에 대한 전문성을 키우겠습니다.<br>
<br>
주니어 개발자로써 제가 가진 열정에 오스템을 더해서 글로벌에 걸맞는 수준있는 개발자가 되도록 노력하겠습니다.